 ...
 */
package org.przybyl.ddj19.concurrency.structuredConcurrency;

import jdk.incubator.concurrent.*;
import static org.przybyl.ddj19.concurrency.virtThreads.VirtThreadsLimits.*;
...
//            result = misusedScoreBoard();
    ...
    static Map<String, Integer> structuredScoreBoard() throws ExecutionException, InterruptedException {
        try (var scope = new StructuredTaskScope.ShutdownOnFailure()) {
            var namesFuture = scope.fork(() -> nameService.getNames());
            var scoresFuture = scope.fork(() -> scoreService.getScores());
            scope.join();
            scope.throwIfFailed();
            var names = namesFuture.resultNow();
            var scores = scoresFuture.resultNow();
            return combine(names, scores);
        }
    }
